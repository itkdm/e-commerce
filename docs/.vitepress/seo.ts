...
export function createSeoHead({ pageData, siteData, title, description, siteUrl }: SeoOptions): HeadConfig[] {
  ...
  if (kind === 'website') {
    head.push([
      ...
      JSON.stringify({
        '@context': 'https://schema.org', '@type': 'WebSite', name: siteData.title,
        url: canonicalUrl, description: description || siteData.description, inLanguage: siteData.lang
      })
    ])
    return head
  }
  ...
  const structuredData: Record<string, unknown> = {
    '@context': 'https://schema.org', '@type': kind === 'article' ? 'Article' : 'CollectionPage',
    headline: title, name: title, description: description || siteData.description,
    url: canonicalUrl, image: socialImageUrl, inLanguage: siteData.lang,
